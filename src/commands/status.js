    ...
    async run(message, args, bot) {
        ...
        server.ping(1000, 757, (err, res) => {
            if (err) {
                ...
                const buff = new Buffer.from(res.favicon.split(',')[1], 'base64');
                const attachment = new discord.MessageAttachment(buff, 'output.png');
                const embed = new discord.MessageEmbed()
                    .setTitle('Server Status')
                    .setColor('PURPLE')
                    .setDescription('Players online: ' + res.players.online + '/' + res.players.max)
